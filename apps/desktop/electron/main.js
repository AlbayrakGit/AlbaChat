...
function createMainWindow(serverUrl) {
  ...
  // --- Navigasyon Takibi ile Otomatik Boyutlandırma ---
  // Bu mantık frontend'den bağımsız olarak URL'ye göre boyutu ayarlar.
  const updateSizeByUrl = (url) => {
    if (!mainWindow) return;
    if (url.includes('/admin')) {
      console.log('Admin detected, expanding to 1200');
    ...
    } else if (url.includes('https://albachat.ernaholding.com') && !url.includes('/admin')) {
      // Sadece ana uygulama içindeyken (login veya chat) küçült
      console.log('Chat/Login detected, shrinking to 800');
      mainWindow.setResizable(true);
  ...
  // --- UI Temizliği: Download Linkini Gizle ---
  mainWindow.webContents.on('did-finish-load', () => {
    mainWindow.webContents.insertCSS(`
      /* Electron içinde indirme linkini gizle */
      a[href*="AlbaChat-Setup.exe"] { display: none !important; }
      /* Giriş sayfasındaki alt bilgi alanını (Desktop Uygulamasını İndir yazan yer) temizle */
...
ipcMain.on('window:resize', (_, { width, height }) => {
  ...
  // Sabit boyutlu pencerede boyut değişimini zorlamak için sınırları geçici olarak ayarla
  mainWindow.setResizable(true);
  mainWindow.setMinimumSize(width, height);
  mainWindow.setMaximumSize(width, height);
  mainWindow.setSize(width, height, true);

  // Ekranın ortasında kalmasını sağla
  mainWindow.center();
